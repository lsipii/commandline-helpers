...
export function colourFormatCliText(text: string, colourIdent: string | Array<string>, disableColouring: boolean = false): string {
    ...
    if (colourIdent instanceof Array) {
        ...
        colourIdent.forEach(function (cI) {
            if (typeof colourEscapes[cI] !== "undefined") {
                escapes += colourEscapes[cI];
            }
        });
        return escapes + text + colourEscapes.Reset;
    } else {
        if (typeof colourEscapes[colourIdent] !== "undefined") {
            return colourEscapes[colourIdent] + text + colourEscapes.Reset;
        }
    }
    return text;
}
